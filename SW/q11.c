...
int main(void)
{
  ...
  *PMC_PCER = PIO_IDENTIFIER; // enable clock for Parallel I/O (so that it responds to inputs)
	...
	*PIO_SODR = GREEN_LED;
	...
	while (1) {
	  if ((*PIO_PDSR & GREEN_SWITCH) != GREEN_SWITCH)
	    stop = 0;
	  else if ((*PIO_PDSR & RED_SWITCH) != RED_SWITCH)
	    stop = 1;
	  
	  if (stop == 0) {
		if ((*PIO_PDSR & OPTO) != OPTO && iter > 0x00040000) {
			DELAY /= 2;
		} else
		  iter++;
